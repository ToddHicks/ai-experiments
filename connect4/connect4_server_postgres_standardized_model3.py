...
This version adds a scaled reward, so the last played piece does not get the same reward as the first. 
I am hoping this will prevent the AI from severe depression.
This should result in early made moves receiving very small positive or negative rewards. Which is appropriate because its the later
moves that have the biggest impact. I am hoping this model will prevent the AI from making the same mistakes repeatedly.

This first attempt, I am going to comment out the logic to also modify reward by turns taken. The idea is we don't want to
diminish late game punishment artificially, and since the earlier moves are already discounted, it shouldn't be needed.
'''
import argparse
import logging
import os
...
class QTable(Base):
    __tablename__ = 'q_table_connect4-6'
...
class GameStats(Base):
    __tablename__ = 'game_stats_connect4-6'
...
def update_q_table(state, action, reward, next_state, turns_played):
    ...
    # Trying with this commented out to avoid double discounts.
